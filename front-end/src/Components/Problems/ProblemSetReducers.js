...
const makeProblemPartNumber = (
  ...
  return {
    [problemPartCoordinates]: {
      ...
      variableNames: [valueName, quantityName, isFormName, operationTypeName],
    },
    ...(isForDisplayResultsOnly
      ? {}
      : {
...
const makeAnswerForm = (
  ...
  k,
  {
    ...
    progressMeter: { correctFirstTime, testingWithoutForm },
  }
) => {
  let problemPartCoordinates = `${offsetString} ${i} ${j} ${k}`;
  let ijk = `${i} ${j} ${k}`;
  ...
  return {
    ...
    [problem_i_progressMeter]: {
      ...
      functionCode: returnState,
      children: [
        gotItRightTheFirstTimeName, // passes if they are right and submission count == 1
        elseStateName, // 'else' is a keyword in js
      ],
      variableNames: [correctFirstTimeName, testingWithoutFormName],
    },
...
const makeProblemPartsForDisplayResults = (ithProblem) => {
  ...
  const { a, b, theirAnswer, actualAnswer, gotItRightTheFirstTime } =
    ithProblem;
...
const makeProblemParts = (ithProblem) => {
  return [
    ...
    },
    {
      ...
      progressMeter: { correctFirstTime: false, testingWithoutForm: false },
    },
  ];
};
...
const makeProblemSet = (state, action) => {
  ...
  let problemSetId = getChildren(
    temporaryState,
    nameOfProblemSetCetagory
  ).length;
...
const isInteger = (state, action) => {
  ...
  return [
    state,
    !isNaN(parseInt(getVariable(state, submissionStateName, "value").value)) ===
      true,
  ];
};
...
const submitValue = (state, action /*e*/) => {
  ...
  newState = setArray(
    ...
    submissionStateName,
    "quantity",
    makeQuantity(newValue, maxValue)
  );
  ...
  newState = set(
    ...
    submissionStateName,
    "submitCount",
    getVariable(newState, submissionStateName, "submitCount").value + 1
  );
  const submitCount = getVariable(
    newState,
    submissionStateName,
    "submitCount"
  ).value;
  ...
  if (submitCount === 1) {
    newState = set(
      ...
      submissionStateName,
      "firstAnswer",
      getVariable(newState, submissionStateName, "value").value
    );
  }
...
const gotItRightTheFirstTime = (state, action) => {
  ...
  if (submitCount === 1 && correct) {
    ...
    const feedbackMessage = getVariable(
      state,
      submissionStateName,
      "feedbackMessage"
    ).value;
...
const solveProblem = (state, action, i, j, k) => {
  ...
  temporaryState = set(
    ...
    submission,
    "submitCount",
    getVariable(temporaryState, submission, "submitCount").value + 1
  );
  ...
  temporaryState = set(
    ...
    submission,
    "firstAnswer",
    getVariable(temporaryState, submission, "value").value
  );
...
const collectProblems = (state, action, i, j, k) => {
  ...
  let gotItRightTheFirstTime = getVariable(
    state,
    progressMeter,
    "correctFirstTime"
  ).value;
  ...
  temporaryState = tableAssign(state, myProblemTable, {
...
const setupForBackend = (state, action) => {
  ...
  const correctProblems = myCompletedProblems.value["problem set table"].filter(
    (problem) => problem.gotItRightTheFirstTime
  ).length;
  // console.log('correctProblems', correctProblems)
  // calculate % of correct problems
  // round to largest whole number
  temporaryState = tableAssign(temporaryState, myCompletedProblems, {
    ...myCompletedProblems.value,
    "problem sets table": {
      nameOfProblemSet: `${offsetString} problem set 0`,
