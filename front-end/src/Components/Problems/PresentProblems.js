...
const getProblems = createSelector(
  (state, location) => getMyProblems(state, location),
  (stuff) => stuff
);
...
const getProblemSetId = createSelector(
  (state, location) => getTheProblemSetId(state, location),
  (stuff) => stuff
);
...
const PresentProblems = (props) => {
  ...
  const problemNumbers = Object.keys(problems).map((numberString) =>
    Number(numberString)
  );
...
export default connect(mapStateToProps, { autoSolve })(PresentProblems);
