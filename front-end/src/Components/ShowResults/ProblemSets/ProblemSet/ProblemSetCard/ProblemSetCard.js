...
const ProblemSetCard = (props) => {
  const {
    Root,
    stateCoordinates: { problemSet, problemId, offsetString },
  } = props;
  ...
  let {
    ...
    theirAnswer,
    actualAnswer,
    gotItRightTheFirstTime,
  } = getCell(Root, "resultsFromBackend").value["problems"][problemSet][
    problemId
  ];
  ...
  return (
    <div>
      <Problems>
        {a} + {b} = {actualAnswer} your answer {"->"} {theirAnswer}{" "}
        {gotItRightTheFirstTime ? "true" : "false"}
      </Problems>
    </div>
  );
};
...
export default connect(mapStateToProps, {})(ProblemSetCard);
